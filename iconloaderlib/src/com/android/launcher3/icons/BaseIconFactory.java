...
public class BaseIconFactory implements AutoCloseable {
    ...
    protected Drawable normalizeAndWrapToAdaptiveIcon(@Nullable Drawable icon,
        ...
        if (shrinkNonAdaptiveIcons && !(icon instanceof AdaptiveIconDrawable) && !customLauncherIconsOn(mContext)) {
    ...
    private static class CenterTextDrawable extends ColorDrawable {
     ...
     /* add by wangqingsong for add custom launcher icons */
    public static boolean customLauncherIconsOn(Context context){
        if(context == null){
          return false;
        }
        String mThemedIcons=Settings.System.getString(context.getContentResolver(),"themed_icons");
            if(!TextUtils.isEmpty(mThemedIcons) && mThemedIcons.equals("true")){
                return false;
        }
        return context.getResources().getBoolean(
              Resources.getSystem().getIdentifier(
                      "hct_config_custom_launcher_icons",
                      "bool",
                      "android"));
    }
     /* add by wangqingsong for add custom launcher icons end*/

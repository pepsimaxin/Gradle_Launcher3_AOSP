...
public class IconProvider {
    ...
    private Drawable loadActivityInfoIcon(ActivityInfo ai, int density) {
        ...
        /*/  modify by wangshuoshuo for add custom launcher icons
        if (icon == null) {
        /*/
        if(icon == null || BaseIconFactory.customLauncherIconsOn(mContext)) {
        //*/
            icon = ai.loadIcon(mContext.getPackageManager());
        }
        return icon;
    }
    //hct add apply theme Calendar
    private Drawable getCalendarDrawable(int iconDpi) {
        String mThemePkgName="";
        Context mThemePkgContext=null;
        try {
            mThemePkgName = Settings.System.getString(mContext.getContentResolver(),"launcher.theme_package");
            
            if(TextUtils.isEmpty(mThemePkgName)){
                mThemePkgName = "com.hct.theme.smart";
             }
            
            mThemePkgContext = mContext.createPackageContext(mThemePkgName, Context.CONTEXT_IGNORE_SECURITY);
        } catch (PackageManager.NameNotFoundException e) {
            Log.d("replaceAppIcon", "applyThemePkg fail, not found package : " + mThemePkgName);
            return null;
        }
        
        
        try {
            return mThemePkgContext.getResources().getDrawableForDensity(
                                                   mThemePkgContext.getResources().getIdentifier("ic_launcher_icon"+getDayHct(), 
                                                   "drawable", mThemePkgName),iconDpi);
        } catch (Resources.NotFoundException e) {
            return null;
        }
    
    }
    //hct add apply theme Calendar end
    @TargetApi(Build.VERSION_CODES.TIRAMISU)
    private Drawable loadCalendarDrawable(int iconDpi, @Nullable ThemeData td) {
        PackageManager pm = mContext.getPackageManager();
        //hct add apply theme Calendar 
            Drawable mHctCalendarDrawable=getCalendarDrawable(iconDpi);
            if(mHctCalendarDrawable!=null){
                return mHctCalendarDrawable;
            }
        //hct add apply theme Calendar end
        try {
	...
	//hct add apply theme Calendar getDayHct add
    static int getDayHct() {
